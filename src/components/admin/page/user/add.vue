...
<script>
    ...
    methods: {
      ...
      // 获取页面固参数据
      getInitData(){
          this.$resource("/api/dict/getall",{items:"moneyPayment,userGrade"}).get().then((data)=>{
              let _data = data.data;
              if(_data.status == 1){
                 this.userRank =  _data.data["userGrade"];
                 this.moneyPayments = _data.data["moneyPayment"];
                 //console.log(this.moneyPayments);
              }    
          },(error)=>{
      ...
      onSubmit() {
         this.$refs.registerForm.validate((valid) => {
           if (valid) {
             this.$http.post("/api/users/regbyroot/", this.form).then((data) => {
                     this.$message({
